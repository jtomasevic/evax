    ...
    register(storeFn: () => Object): () => any {
        ...
        const useStore = (customConfig?: Array, ...actions: Function): any => {
            ...
            if (Array.isArray(customConfig)) {
                ...
                cmpSetState = customConfig[1];
                classState = customConfig[2];
            }
            // let state = null;
            if (!Array.isArray(customConfig)) {
                ...
                setState = (newState) => {
                    ...
                    // const hm = { ...classState, ...newState };
                    // classState = hm;
                    // console.log('>>> merged state', hm);
                    const mergedState = { ...store, ...newState };
                    console.log('>>> mergedState', mergedState, storeFn.name);

                    cmpSetState.call(component, { classState, [storeFn.name]: mergedState });
                    // cmpSetState(newState);
                    // const storeToSet = { [storeFn.name]: newState[storeFn.name] };
                    // console.log('Setting store >>> ', storeToSet);
                    // component.setState(storeToSet);
                    // setFun[1].call(setFun[0], storeToSet);
                    // setFun[0]({ [storeFn.name]: newState[storeFn.name] });
                };
            }
