    ...
    register(storeFn: () => Object): () => any {
        ...
        const useStore = (customConfig?: Array, ...actions: Function): any => {
            ...
            if (Array.isArray(customConfig)) {
                ...
                console.log('ccc>', classState, this.stores);
            }
            // let state = null;
            if (!Array.isArray(customConfig)) {
                ...
                setState = (newState) => {
                    console.log('c>> classState', classState);
                    console.log('>>> old value', store);
                    console.log('>>> new state', newState);
                    state = { ...this.stores };
                    state[storeFn.name] = newState;
                    // classState.state = state;
                    console.log('>>> mergedState', state, storeFn.name);
                    // classState = { ...classState, [storeFn.name]: mergedState };
                    cmpSetState.call(component, state);
                    console.log('>>> set state called', state, storeFn.name);
                };
            }
